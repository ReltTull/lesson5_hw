...
public class BackUper {
    ...
    public static void createBackup() throws IOException {
        ...
        // Копируем каждый файл в резервную папку с помощью метода copy(), позволяющего производить копирование с
        // опциями. В данном случае - замещая имеющиеся файлы в back up директории.
        if (files != null) {
            for (File file : files) {
                if (file.isFile()) {
